...
| ------------ | ----------- |
| `undefined` to `*`, optional, or Set `enum` | For Set `enum`, results into an empty set. For other types, results into `undefined`. |
...
| From non optional `T` constant to `T?` | Results into the same constant but wrapped in the `Optional` type. |
